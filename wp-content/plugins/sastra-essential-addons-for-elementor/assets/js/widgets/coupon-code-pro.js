...
(function ($) {
	...
	var WidgetCouponCodeHandler = function($scope, $) {
        ...
        var el = $scope[0].querySelector('.tmpcoder-coupon-code');
        var data = JSON.parse(el.getAttribute('data-tmpcoder_cc_settings')),
            ...
            let UniqSCls = '.tmpcoder-widget-' + data.id;
			...
			if(cpnType == 'standard') {
				let UniqCls = '.tmpcoder-widget-' + data.id,
					UniqId = '#tmpcoder-widget-' + data.id,
					UniqId1 = '\#tmpcoder-widget-' + data.id,
					Btnlink = '.tmpcoder-widget-'+data.id+' .coupon-btn-link',
				...
				if(acnType == 'popup') {					
					...
					jQuery(Btnlink).on("click", function(e){						
						...
						jQuery(UniqCls).find(".full-code-text").addClass("tmpcoder-code-popup");
						copycodebtn();
					});
					...
					jQuery(".tmpcoder-ccd-closebtn, .copy-code-wrappar").on("click", function(){
						closeModal();
					});
					...
					function copycodebtn(){
					 ...
					 if($(el).hasClass("tmpcoder-tab-cop-rev")){
						copycodebtn();
						jQuery(Btnlink).on("click", function(e){							
							//window.open(window.location+UniqId1,"_blank'");

							let decodedURL = htmlDecode(extlink);
							setTimeout(function(){
					...
					var standardStyle = (data && data.standard_style) ? data.standard_style : '',
						style6ResetMs = (data && data.style6_copied_reset_ms) ? parseInt(data.style6_copied_reset_ms, 10) : 2000;
					if (isNaN(style6ResetMs) || style6ResetMs < 100) {
						style6ResetMs = 2000;
					}
					...
					if( lengthCheck.length > 0 ){
						lengthCheck[0].addEventListener("click", function(e) {
							...
							let Codecp = (data && data.coupon_code) ? data.coupon_code : '',
								...
								if(this.classList.contains("tmpcoder-hl-links")){
									var hlset = JSON.parse(this.dataset.hlset);
										if( hlset.length > 0 ){
											hlset.forEach(element => {
												if( element && element !== '#' && element !== 'javascript:void(0)' ){
													window.open(element, '_blank');	
												}
											});
										}
								}else{
									var dynamicURL = this.getAttribute('href') || '';
										if (dynamicURL && dynamicURL !== '#' && dynamicURL !== 'javascript:void(0)') {
											window.open(dynamicURL, '_blank');
										}
								}

								if (standardStyle === 'style-6') {
									let copyTextValue = Codecp;
									let $inner = jQuery(this).closest('.coupon-code-inner');
									copyInlineText(copyTextValue).then(function() {
										$inner.addClass('is-copied');
										let copiedTimer = $inner.data('style6CopiedTimer');
										if (copiedTimer) {
											clearTimeout(copiedTimer);
										}
										let nextTimer = setTimeout(function() {
											$inner.removeClass('is-copied');
										}, style6ResetMs);
										$inner.data('style6CopiedTimer', nextTimer);
									});
								} else {
									jQuery(this).closest(UniqCls +' .coupon-code-inner').replaceWith( Ccdval );
									jQuery(UniqCls).find(".tmpcoder-coupon-code, .coupon-code-inner").css('width', CstStdBtnWidth + '%');
				...
				var peel = new Peel( ".tmpcoder-widget-"+data.id , {
	...
	$(window).on('elementor/frontend/init', function () {
		elementorFrontend.hooks.addAction('frontend/element_ready/tmpcoder-coupon-code-pro.default', WidgetCouponCodeHandler);
	});
})(jQuery);